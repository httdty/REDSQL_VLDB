...
from schema_prune.bridge_content_encoder import get_database_matches
from sql_metadata import Parser
from tqdm import tqdm

sql_keywords = [
    'select', 'from', 'where', 'group', 'order', 'limit', 'intersect', 'union',
...
def parse_option():
    ...
    parser.add_argument('--table_path', type=str, default="./datasets/spider/tables.json")
    parser.add_argument('--input_file', type=str, default="./datasets/spider/train_spider.json")
    parser.add_argument('--db_path', type=str, default="./datasets/spider/database")
...
def get_db_contents(question, table_name_original, column_names_original, db_id, db_path):
    ...
    for column_name_original in column_names_original:
        matches = get_database_matches(
            ...
            os.path.join(db_path, db_id, f"{db_id}.sqlite")
        )
...
def extract_skeleton(sql, db_schema):
    ...
    sql_skeleton = " ".join(sql_skeleton.split())
...
def human_fix(data):
    if data['query'] == 'SELECT T1.company_name FROM Third_Party_Companies AS T1 ' \
        ...
        data['query'] = 'SELECT T1.company_type FROM Third_Party_Companies AS T1 ' \
                        'JOIN Maintenance_Contracts AS T2 ON T1.company_id  =  T2.maintenance_contract_company_id ' \
                        'ORDER BY T2.contract_end_date DESC LIMIT 1'
        data['query_toks'] = ['SELECT', 'T1.company_type', 'FROM', 'Third_Party_Companies', 'AS', 'T1', 'JOIN',
                              ...
                              'LIMIT', '1']
        data['query_toks_no_value'] = ['select', 't1', '.', 'company_type', 'from', 'third_party_companies', 'as', 't1',
                                       'join', 'maintenance_contracts', 'as', 't2', 'on', 't1', '.', 'company_id', '=',
                                       't2', '.', 'maintenance_contract_company_id', 'order', 'by', 't2', '.',
                                       'contract_end_date', 'desc', 'limit', 'value']
...
def main(opt):
    dataset = json.load(open(opt.input_file))
    ...
    for data in tqdm(dataset):
        ...
        preprocessed_data = {
            "question": question, "db_id": db_id, "sql": sql, "norm_sql": norm_sql,
            "sql_skeleton": sql_skeleton, "db_schema": [], "pk": db_schemas[db_id]["pk"],
            "fk": db_schemas[db_id]["fk"], "table_labels": [], "column_labels": []
        }
    ...
    with open(opt.output_file, "w") as f:
        json.dump(preprocessed_dataset, f, indent=2, ensure_ascii=False)

...
from red.extract_comm.process import tokenize, get_schema, get_tables_with_alias, Schema, get_sql
...
def eval_from(pred, label):
    pred_tab = pred["from"]['table_units']
    label_tab = label["from"]['table_units']
    predt_total = len(pred_tab)
    labelt_total = len(label_tab)
    cnt = 0

    for unit in pred_tab:
        if unit in label_tab:
            cnt += 1
            label_tab.remove(unit)

    pred_con = pred["from"]['conds']
    label_con = label["from"]['conds']
    predc_total = len(pred_con)
    labelc_total = len(label_con)
    for unit in pred_con:
        if unit in label_con:
            cnt += 1
            label_con.remove(unit)
    label_total = labelt_total + labelc_total
    pred_total = predt_total + predc_total
